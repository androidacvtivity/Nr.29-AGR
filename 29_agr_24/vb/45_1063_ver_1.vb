Sub InserareFilialaSiColorare()
    ...
    For i = 1 To lastRow
        ...
        If Len(cuiio) > 0 Then
            ...
            primele2C = Left(Trim(CStr(ws.Cells(i, 3).Value)), 2)
            ...
            For j = 1 To lastRow
                If i <> j Then
                    If InStr(1, ws.Cells(j, 1).Value, cuiio, vbTextCompare) > 0 Then
                        ...
                        ws.Cells(j, 8).Value = "FILIALA - " & cuiioTrimmed & " - " & ws.Cells(j, 3).Value
                        ...
                        If Left(Trim(CStr(ws.Cells(j, 3).Value)), 2) = Right(cuiioTrimmed, 2) Then
                            ...
                            Dim hText As String
                            hText = "FILIALA - " & cuiioTrimmed & " - " & ws.Cells(j, 3).Value
                            ws.Cells(j, 8).Value = hText
                            ' Coloram ultimele 2 caractere din codul CUIIO in rosu
                            With ws.Cells(j, 8).Characters(Start:=Len("FILIALA - " & cuiioTrimmed) - 1, Length:=2).Font
                                .Color = RGB(255, 0, 0) ' Rosu
                                .Bold = True
                            End With
            ...
            If (Len(cuiio) = 8 Or Len(cuiio) = 9 Or Len(cuiio) = 10) And ultimele2A = primele2C Then
                ...
                ws.Cells(i, 8).Value = "FILIALA - " & cuiio & " - " & ws.Cells(i, 3).Value
